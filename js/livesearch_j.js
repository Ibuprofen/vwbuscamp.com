		...
		if(xmlHttp.responseXML.getElementsByTagName('item')) {
			...
			if(items.length > 0) {
				for(var i = 0; i < items.length; i++) {
					...
					displayObj = document.getElementById(displayId);
					displayObj.innerHTML = displayObj.innerHTML + "<span repvalue=\"" + value + "\" onmouseover=\"this.style.background='yellow'\" onmouseout=\"this.style.background='none'\" onclick=\"document.getElementById('" + hiddenId + "').value=this.getAttribute('repvalue');document.getElementById('" + inputId + "').value=this.innerHTML;document.getElementById('" + displayId + "').style.display='none'\">" + name + "</span><br />";
